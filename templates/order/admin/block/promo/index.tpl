            ...
            $(this).ajaxSubmit({
                success: function(data) {
                    if (data=='ok') {
                        ...
                        notice('Настройки блока акции успешно изменены');
                    } else {
...
<div class="cats">
    ...
    <span style="background:#ffffff;">Акция</span>
...
<div class="catExt oEditor">
    ...
    <table><tbody>
        ...
        <tr>
            ...
            <td class="pictureBlock"><form action="admin/block/promo/add" method="post">
			    <!-- BEGIN no_img -->
				<div class="addImage">
				    <span class="oFileLink add"><i>Прикрепить</i><input type="file" name="file" size="40" rel="admin/block/promo/add"/></span>
				</div>
				<!-- END no_img -->
				<!-- BEGIN img -->
				<div class="changeImage">
				    <img src="images/stuff/promo.png?rand={RAND}" alt="" title="" style="border:1px solid gray;padding:3px;"/>
				    <span class="oFileLink add"><i>Поменять</i><input type="file" name="file" size="40" rel="admin/block/promo/add"/></span>
				    <span class="oFileLink delete" rel="admin/block/promo/delete"><i>Удалить</i></span>
				</div>
				<!-- END img -->
            </form></td>
        </tr>
    ...
    <div class="settings"><form action="admin/block/promo/settings" method="post">
        <div class="oT2 oEditItem"><!-- INCLUDE admin/block/promo/settings.tpl --></div>
    </form></div>
</div>
