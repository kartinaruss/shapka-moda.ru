            ...
            $(this).ajaxSubmit({
                success: function(data) {
                    if (data=='ok') {
                        ...
                        notice('Настройки блока спецпредложения успешно изменены');
                    } else {
...
<div class="cats">
    ...
    <span style="background:#ffffff;">Специальное предложение</span>
    <a href="admin/stuff/cert">Сертификаты</a>
</div>
<div class="catExt oEditor">
    <div class="oT2 oEditItem" style="display:none;">
    <table><tbody>
	    <tr><td colspan="2"><h3>&nbsp;</h3></td></tr>
        <tr>
            <td><span>Картинка:</span></td>
            <td class="pictureBlock"><form action="admin/block/special/add" method="post">
			    <!-- BEGIN no_img -->
				<div class="addImage">
				    <span class="oFileLink add"><i>Прикрепить</i><input type="file" name="file" size="40" rel="admin/block/special/add"/></span>
				</div>
				<!-- END no_img -->
				<!-- BEGIN img -->
				<div class="changeImage">
				    <img src="images/stuff/special.png?rand={RAND}" alt="" title="" style="border:1px solid gray;padding:3px;"/>
				    <span class="oFileLink add"><i>Поменять</i><input type="file" name="file" size="40" rel="admin/block/special/add"/></span>
				    <span class="oFileLink delete" rel="admin/block/special/delete"><i>Удалить</i></span>
				</div>
				<!-- END img -->
            </form></td>
        </tr>
    ...
    <div class="settings"><form action="admin/block/special/settings" method="post">
        <div class="oT2 oEditItem"><!-- INCLUDE admin/block/special/settings.tpl --></div>
    </form></div>
</div>
