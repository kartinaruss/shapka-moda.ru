<table id="list">
    <thead><tr class="nodrop nodrag">
        ...
        <td><s><font size="-1">Старая цена</font></s></td>
        <td>Цена</td>
    </tr></thead>
    <tbody>
    <!-- BEGIN product -->
	<tr id="{product.ID}">
        ...
        <!-- BEGIN tab_search --><td style="width:0px;border:0"></td><!-- END tab_search -->
	    <td><div class="actions">
            ...
            <a href="admin/product/extra/{product.ID}" class="extra" title="Дополнительно"><i>Дополнительно</i></a>
	    ...
	    <td><!-- BEGIN product.picture --><div class="img {product.LABEL}"></div><img src="images/product/s/{product.picture.SRC}" alt="" width="180" /></div><!-- END product.picture --></td>
	    <td class="iName">{product.NAME}</td>
        <td>{product.CODE}</td>
        <td>{product.BRIEF}</td>
        <td width="100"><input type="text" name="price_old" value="{product.PRICE_OLD}" size="6"/></td>
        <td width="100"><input type="text" name="price" value="{product.PRICE}" size="6"/></td>
	</tr>
    <!-- END product -->
    </tbody>
</table>
