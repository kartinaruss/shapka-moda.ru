...
{
    const DIR_PICTURES = 'product/category';
    const DIR_PICTURES_BIG = 'product/category/l';
    ...
    function addAction(array $params)
    {
        ...
        if ($form->isSubmit() && $form->isValid())
        {
            ...
            if ($data->pictureKey)
            {
                ...
                $destBig= DIR_IMAGES.'/'.self::DIR_PICTURES_BIG.'/'.$filename;
                if (is_file($source))
                {
                    copy($source, $destBig);
                    $image = $this->model->image->load($source);
                    $image->framing(182,182)->save($dest,IMAGETYPE_PNG);
                    unlink($source);
    ...
    function editAction(array $params)
    {
        ...
        $form->setValues(array('my_key'=>$category->key));
        if ($form->isSubmit() && $form->isValid())
        {
            $data = new Entity($form->getData());
            $dataToInsert = array(
            ...
            if ($data->pictureKey)
            {
                ...
                $oldBig = DIR_IMAGES.'/'.self::DIR_PICTURES_BIG.'/'.$category->file;
                if (is_file($source))
                {
    ...
    function pictureAction(array $params)
    {
        ...
        if ($form->isSubmit() && $form->isValid())
        {
             ...
             * Сохраняем картинку
             */
            move_uploaded_file($data->file->tmp_name,$dest);
