...
class App_Admin_Testimonial extends Controller
{
    ...
    function tableAction(array $params = array())
    {
        ...
        foreach ($testimonials as $testimonial)
        {
            ...
            if ($testimonial->file)
            {
                $src = DIR_FILES.'/'.$testimonial->file;
                if (is_file($src))
                {
                    $this->tpl->assignBlockVars('testimonial.audio',array(
    ...
    function addAction(array $params)
    {
        ...
        if ($form->isSubmit() && $form->isValid())
        {
             ...
             * Сохраняем файл
             */
            if ($data->file2 && $data->file2->tmp_name)
            {
                $dir = DIR_FILES.'/';
                $filename = $id.substr(uniqid(),-5).'.mp3';
                copy($data->file2->tmp_name,$dir.$filename);
                $this->model->testimonial->edit(array(
                    'file' => $filename
                ),$id);
            }
    ...
    function editAction(array $params)
    {
        ...
        if ($form->isSubmit() && $form->isValid())
        {
             ...
             * Сохраняем логотип
             */
            if ($data->pictureKey && !$data->picture_delete)
            {
                $source = DIR_IMAGES.'/'.self::DIR_PICTURES_TEMP.'/'.$data->pictureKey;
                $filename = $id.substr(uniqid(),-5).'.jpg';
                $destS = DIR_IMAGES.'/'.self::DIR_PICTURES_S.'/'.$filename;
            ...
            if ($data->picture_delete)
            {
                ...
                if (is_file($src))
                    unlink($src);
                $dataToInsert['picture'] = '';
            }
            /**
             * Сохраняем файл
             */
            if ($data->file2 && $data->file2->tmp_name && !$data->file_delete)
            {
                $dir = DIR_FILES.'/';
                $filename = $id.substr(uniqid(),-5).'.mp3';
                copy($data->file2->tmp_name,$dir.$filename);
                if (is_file($dir.$item->file))
                   unlink($dir.$item->file);
                $dataToInsert['file'] = $filename;
            }
            if ($data->file_delete)
            {
                $src = DIR_FILES.'/'.$item->file;
                if (is_file($src))
                    unlink($src);
                $dataToInsert['file'] = '';
            }
        ...
        if ($item->file)
        {
            $this->tpl->assignBlockVars('mp3',array('SRC'=>$item->file));
        }
    }
