...
class Model_Product extends Model_Default
{
	...
	function getByIdsU($ids)
    {
        ...
        if (false === ($data = $this->cache->get($cacheKey))) {
            ...
            $_data = (array)$this->db->selectPage($total,
                "SELECT _products.*, max(_product_structure.category_id) as category_id FROM ?_{$this->table}
                LEFT JOIN _product_structure ON product_id = id
                WHERE id IN (?a)
                GROUP BY id",
                $_ids
            );
    ...
    function getByCategory($category_id, &$total=null, $start=0, $count=1000, $disabled = false)
    {
        ...
        {
            $data['items'] = (array)$this->db->selectPage($total,
                "SELECT DISTINCT p.*, 0 as top FROM ?_{$this->table} p
    ...
    function getByCategoryHit($category_id, &$total=null, $start=0, $count=1000, $disabled = false)
    {
        ...
        if (false === ($data = $this->cache->get($cacheKey)))
        {
            ...
            "SELECT DISTINCT p.*, 1 as top FROM ?_{$this->table} p
                JOIN ?_{$this->structure->getTable()} s ON s.`product_id`=p.`id`
        ...
        $items = (array)$this->db->selectPage($total,
            "SELECT DISTINCT p.*, 0 as top FROM ?_{$this->table} p
                JOIN ?_{$this->structure->getTable()} s ON s.`product_id`=p.`id`
                WHERE s.`category_id`=?d { AND p.`disabled`=? }
    ...
    {//new for hits
        ...
        {	
			...
			$hits = (array)$this->db->selectPage($total,
				"SELECT *, 1 as top FROM ?_{$this->table}
					WHERE `main`=1 AND disabled=0
					ORDER BY order_count DESC, position_main ASC, price ASC, brief ASC LIMIT 12"
			);
			...
			$items = (array)$this->db->selectPage($total,
				"SELECT *, 0 as top FROM ?_{$this->table}
